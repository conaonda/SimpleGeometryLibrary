...
{
	[Serializable]
	public class KdTree<TKey, TValue> : IKdTree<TKey, TValue> where TKey : IComparable, IConvertible
    {
		...
		public bool Add(TKey[] point, TValue value)
		{
			...
			else
			{
				...
				do
				{
				    ...
				    if (point.Length == parent.Point.Length &&
                        !point.Where((t, i) => (Number<TKey>)t != parent.Point[i]).Any())
				    {
				        return false;
				    }

					// Which side does this node sit under in relation to it's parent at this level?
					int compare = point[dimension].CompareTo(parent.Point[dimension]);
		...
		public void RemoveAt(TKey[] point)
		{
		    ...
		    if (point.Length == this.root.Point.Length &&
		        !point.Where((t, i) => (Number<TKey>) t != this.root.Point[i]).Any())
		    {
		        node = root;
		        root = null;
		        Count--;
			...
			do
			{
				...
				int compare = point[dimension].CompareTo(node.Point[dimension]);

				if (node[compare] == null)
					// Can't find node
					return;

                if (point.Length == node.Point.Length &&
                    !point.Where((t, i) => (Number<TKey>)t != node.Point[i]).Any())
                {
		...
		public bool TryFindValueAt(TKey[] point, out TValue value)
		{
			...
			do
			{
				...
				else if (point.Length == parent.Point.Length && !point.Where((v,i) => (Number<TKey>)v != parent.Point[i]).Any())
				{
					value = parent.Value;
					return true;
				}
		...
		private void SortNodesArray(KdTreeNode<TKey, TValue>[] nodes, int byDimension, int fromIndex, int toIndex)
		{
			for (var index = fromIndex + 1; index <= toIndex; index++)
			{
				...
				while (true)
				{
					...
					if (b.Point[byDimension].CompareTo(a.Point[byDimension]) < 0)
					{
						nodes[newIndex - 1] = b;
						nodes[newIndex] = a;
					}
